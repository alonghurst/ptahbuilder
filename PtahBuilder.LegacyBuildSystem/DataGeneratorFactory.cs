...
namespace PtahBuilder.LegacyBuildSystem;
...
public class DataGeneratorFactory
{
    ...
    public void Process()
    {
        var generatorTypes = _typesToGenerate.Select(t => new { BaseData = t, generatorType = OperationReflectionHelper.FindBaseDataGeneratorType(t) })
            .ToDictionary(t => t.BaseData, t => t.generatorType);

        var metadataResolverTypes = _typesToGenerate.Select(t => new { BaseData = t, metadataResolverType = OperationReflectionHelper.FindBaseDataMetadataResolverType(t) })
    ...
    private void ProcessOperations(Dictionary<Type, ProcessedType> processedTypes)
    {
        ...
        {
            var operationProviderType = OperationReflectionHelper.FindOperationProviderType(context.Key);
            ...
            var operationTypes = OperationReflectionHelper.FindOperationTypes(context.Key);
